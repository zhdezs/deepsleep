...
| | |
| --- | --- |
| 当前版本 | **2.2.0**（桌宠终于能点、能拖、能右键了：以前桌宠窗口是系统「静态控件」，Windows 会把鼠标消息一律判成穿透，单击 / 拖拽 / 右键全落到桌面上；现在按鲸鱼像素的透明度自己算命中区域，点鲸鱼有反应，鲸鱼周围的透明处照样点得到桌面。工具结果改成可展开卡片：联网搜索不展开只显示「搜到 N 篇资料」，点开看标题 / 链接 / 摘要；运行命令、编辑文件同理。深度研究不再只是 chat 模式的专属。AI 回复被长度上限截断时自动接着写，不用再手动说「继续」）。新增官网与免安装网页版（内置免费模型矩阵：GLM-4.7-Flash 聊天 / GLM-4.6V-Flash 看图 / CogView-3-Flash 画图）与内核版 `deepsleep-core.exe`：只装一个几十 MB 的内核，就能用任意浏览器操控这台电脑 |
